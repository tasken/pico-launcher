...
- `gameLanguage` - Language that games are started in: `auto` (default), `japanese`, `english`, `french`, `german`, `italian`, `spanish`, `chinese` or `korean`. This is separate from `language`, which only affects Pico Launcher itself.
